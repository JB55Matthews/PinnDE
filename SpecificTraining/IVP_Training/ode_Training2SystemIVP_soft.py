...
from ...ode_ModelFuncs import build_model_system2_IVP
import ast

#The main trainign function
@tf.function
def train_network_2system_IVP(odes, inits, order, model, gamma, eqnparam):
    """
    ...
    with tf.GradientTape() as tape:
      ...
      DEloss = tf.reduce_mean(tf.square(eqn1parse)) + tf.reduce_mean(tf.square(eqn2parse))

      # Define the initial value loss
      with tf.GradientTape(persistent=True) as tape3:
        tape3.watch(t_init)
        [u_init_pred, x_init_pred] = model(t_init)
        if order[0] > 1:
...
def PINNtrain_2System_IVP(de_points, inits, order, t0, epochs, eqn, net_layers, net_units, model):
  """
  ...
  Packages data correctly and calls train_network_2system_IVP in executing training routine, and handles
  ...
  if order[0] == 1:
    if order[1] == 1:
      inits = np.column_stack([t_init, u_init, x_init])
    elif order[1] == 2:
      inits = np.column_stack([t_init, u_init, x_init, xt_init])
    elif order[1] == 3:
      inits = np.column_stack([t_init, u_init, x_init, xt_init, xtt_init])
  elif order[0] == 2:
    if order[1] == 1:
      inits = np.column_stack([t_init, u_init, ut_init, x_init])
    elif order[1] == 2:
      inits = np.column_stack([t_init, u_init, ut_init, x_init, xt_init])
    elif order[1] == 3:
      inits = np.column_stack([t_init, u_init, ut_init, x_init, xt_init, xtt_init])
  elif order[0] == 3:
    if order[1] == 1:
          inits = np.column_stack([t_init, u_init, ut_init, utt_init, x_init])
    elif order[1] == 2:
          inits = np.column_stack([t_init, u_init, ut_init, utt_init, x_init, xt_init])
    elif order[1] == 3:
          inits = np.column_stack([t_init, u_init, ut_init, utt_init, x_init, xt_init, xtt_init])
  ...
  if model ==None:
    model = build_model_system2_IVP(net_layers, net_units)
  ...
  for i in range(epochs):
    ...
    for (des, inits) in ds:
      ...
      # Train the network
      DEloss, IVloss, grads = train_network_2system_IVP(des, inits, order, model, gamma, eqn)
