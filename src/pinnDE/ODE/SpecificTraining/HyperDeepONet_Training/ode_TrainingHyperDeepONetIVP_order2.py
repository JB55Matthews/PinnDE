...
class HardConstraint(nn.Module):
  ...
  def __call__(self, inputs):
    t, nn = inputs
    return self.u0 + self.ut0*(t-self.t0) + ((t-self.t0)/(self.tfinal-self.t0))**2*nn
...
class DeepONet(nn.Module):
  ...
  def __call__(self, t, u, ut):
    ...
    params = HyperBranchNet(self.layers, 2*self.units, self.lora_rank, 2*no_trunk_params)(z)
    batch_size = params.shape[0]
    out = []
    t = jnp.reshape(t, (-1,1))
    for i in range(batch_size):
      cparams = reconstruct_pytree(tree_info, shapes, params[i,])
      outc= trunk_net.apply(cparams, t[i:i+1,])
      out.append(outc[:,0])
    out = jnp.array(out)
    out = HardConstraint(self.t0, self.tfinal, u0, ut0)([t, out])
...
def train_step(params, pdes, z, zt):
  ...
  def q_model(t, z, zt, params):
    return deeponet.apply(params, t, z, zt)[0]
  def u0(t, z, zt, params):
    return jax.vmap(q_model, [0, 0, 0, None])(t, z, zt, params)
  def q_t(t, z, zt, params):
    return jax.vmap(jax.grad(q_model, 0), [0, 0, 0, None])(t, z, zt, params)
  def q_tt(t, z, zt, params):
    return jax.vmap(jax.grad(jax.grad(q_model, 0), 0), [0, 0, 0, None])(t, z, zt, params)
  def loss(params, t, z, zt):
    #u = deeponet.apply(params, t, z, zt)
    u = u0(t, z, zt, params)
    ut = q_t(t, z, zt, params)
    utt = q_tt(t, z, zt, params)
...
def startTraining(eval_points, inits, order, t, N_sensors, sensor_range, epochs, net_layers, net_units, eqn):
  ...
  # Initialize the model parameters
  params = deeponet.init(jax.random.PRNGKey(0), jnp.ones((1,)), jnp.ones((1,)), jnp.ones((1,))) ###
