...
class DeepONet(nn.Module):
  ...
  def __call__(self, t, u):
    ...
    trunk_x2 = trunk_net[:,2*self.units:3*self.units]
    trunk_x3 = trunk_net[:,3*self.units:]

    branch_x1 = branch_net[:,:2*self.units]
    branch_x2 = branch_net[:,2*self.units:3*self.units]
    branch_x3 = branch_net[:,3*self.units:]
