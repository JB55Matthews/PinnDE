...
def train(pde_points, inits, u_sensors, setup_boundries, t_order, epochs, model, eqn, N_iv, extra_ders):
  """
  Main function called by PINNtrainSelect_DeepONet_tx when solving equation in tx with dirichlet boundries with soft constraint.
...
#   opt = tf.keras.optimizers.Adam(lr_schedule, beta_1 = 0.95, beta_2 = 0.99)
  ...
  for i in range(epochs):
    ...
    if (np.mod(i, 100)==0):
      print(f"Total loss {i}th epoch: {loss: 6.4f}.")
