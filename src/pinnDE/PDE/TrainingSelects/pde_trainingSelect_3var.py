...
def PINNtrainSelect_txy(pde_points, init_points, epochs, eqn, t_order, N_pde, N_iv, setup_boundries, model, constraint, flag): #selecting which to train
   boundry_type = setup_boundries[0]

   if flag == "ICBCP-txy":
      if boundry_type == "periodic_timeDependent":
         if constraint == "soft":
            return trainICBCP3_periodic_soft.PINNtrain(pde_points, init_points, t_order, setup_boundries, epochs, eqn, N_pde, N_iv, model)
         elif constraint == "hard":
               return trainICBCP3_periodic_hard.PINNtrain(pde_points, setup_boundries, epochs, eqn, N_pde, N_iv, model)
         
      elif boundry_type == "dirichlet_timeDependent":
         if constraint == "soft":
            return trainICBCP3_dirichlet_soft.PINNtrain(pde_points, init_points, t_order, setup_boundries, epochs, eqn, N_pde, N_iv, model)
         elif constraint == "hard":
         ...
         if constraint == "soft":
            return #trainIBCP_neumann_soft.PINNtrain(pde_points, init_points, t_order, setup_boundries, epochs, eqn, N_pde, N_iv, model)
