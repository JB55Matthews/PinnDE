...
def train_network(pdes, uIn, bcs, opt, model, equation, N_bc, extra_ders):
  ...
  Args:
    ...
    N_bc (int): Number of randomly sampled collocation points along boundries
  ...
  with tf.GradientTape() as tape:
    ...
    #Define BC losss
    xbc_left = x_bc[:N_bc]
    xbc_right = x_bc[N_bc:2*N_bc]
    xbc_lower = x_bc[2*N_bc:3*N_bc]
    xbc_upper = x_bc[3*N_bc:]

    ybc_left = y_bc[:N_bc]
    ybc_right = y_bc[N_bc:2*N_bc]
    ybc_lower = y_bc[2*N_bc:3*N_bc]
    ybc_upper = y_bc[3*N_bc:]

    uleft = u_bc[:N_bc]
    uright = u_bc[N_bc:2*N_bc]
    ulower = u_bc[2*N_bc:3*N_bc]
    uupper = u_bc[3*N_bc:]
    ...
    BCloss = tf.reduce_mean(tf.square(uleft-uleft_bc_pred) + tf.square(uright-uright_bc_pred) +
                            tf.square(ulower-ulower_bc_pred) + tf.square(uupper-uupper_bc_pred))
...
def train(pde_points, u_sensors, setup_boundries, epochs, model, eqn, N_bc, extra_ders):
  """
  Main function called by PINNtrainSelect_DeepONet_xy when solving equation in xy with dirichlet boundries with soft constraint.
  ...
  Args:
    ...
    N_bc (int): Number of randomly sampled collocation points along boundries which DeepONet uses in training.
    extra_ders (list): Extra derivatives needed to be computed for user equation
