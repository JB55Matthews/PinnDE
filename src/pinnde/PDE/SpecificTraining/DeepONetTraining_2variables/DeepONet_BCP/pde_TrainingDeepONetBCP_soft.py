...
def train_network(pdes, uIn, bcs, opt, model, equation, N_bc, extra_ders, boundary_type):
  """
...
#   opt = tf.keras.optimizers.Adam(lr_schedule, beta_1 = 0.95, beta_2 = 0.99)
  ...
  for i in range(epochs):
    ...
    for (pdes, usensor) in ds:
      ...
      # Train the network
      loss, PDloss, BCloss = train_network(pdes, usensor, bcs, opt, model, eqn, N_bc, extra_ders, boundary_type)
