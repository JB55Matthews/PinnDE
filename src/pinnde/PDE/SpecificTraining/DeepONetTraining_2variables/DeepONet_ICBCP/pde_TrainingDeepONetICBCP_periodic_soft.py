...
def train_network(pdes, inits, uIn, t_order, opt, model, equation, extra_ders):
  """
  ...
  return loss, PDloss, IVloss

def train(pde_points, inits, u_sensors, t_order, epochs, model, eqn, N_iv, extra_ders):
  """
  Main function called by PINNtrainSelect_DeepONet_tx when solving equation in tx with periodic boundries with soft constraint.
...
#   opt = tf.keras.optimizers.Adam(lr_schedule, beta_1 = 0.95, beta_2 = 0.99)
  ...
  for i in range(epochs):
    ...
    for (pdes, usensor) in ds:
      ...
      # Train the network
      loss, PDloss, IVloss = train_network(pdes, inits, usensor, t_order, opt, model, eqn, extra_ders)
    ...
    if (np.mod(i, 100)==0):
      print("PDE loss, IV loss in {}th epoch: {: 6.4f}, {: 6.4f}.".format(i, PDloss.numpy(), IVloss.numpy()))
