...
def defineCollocationPoints_xy(x_bdry, y_bdry, N_pde):
  """
  Function which generates collocation points along x and y.
  ...
  pde_points = xy_min + (xy_max - xy_min)*lhs(2, N_pde)
  x_pde = pde_points[:,0]
  y_pde = pde_points[:,1]
  pdes = np.column_stack([x_pde, y_pde]).astype(np.float64)
  return pdes
...
def defineCollocationPoints_DON_xy(x_bdry, y_bdry, N_pde, N_bc, N_sensors, sensor_range):
  """
  Function which generates collocation points along x and y, and which generates sensors for DeepONet.

  Args:
    x_bdry (list): list of two elements, the interval of x to be solved on.
    y_bdry (list): list of two elements, the interval of y to be solved on.
    N_pde (int): Number of randomly sampled collocation points along t and x which PINN uses in training.
    N_bc (int): Number of randomly sampled collocation points along boundries which PINN uses in training.
    N_sensors (int): Number of sensors in which network learns over. 
    sensor_range (list): Range in which sensors are sampled over.
  ...
  y_pde = pde_points[:,1]
  pdes = np.column_stack([x_pde, y_pde]).astype(np.float64)
  usensors = np.random.uniform(float(sensor_range[0]), float(sensor_range[1]), size=(N_sensors, N_bc))
  return pdes, usensors
