...
class pde_xy_solution:
    ...
    def __init__(self, eqn, setup_boundries, x_bdry, y_bdry, N_pde, epochs, net_layers, net_units, constraint, 
        ...
        if model == None:
            self._model = pde_ModelFuncs.select_model_xy(x_bdry, y_bdry, net_layers, net_units, constraint, setup_boundries)

        self._pde_points = pde_Points.defineCollocationPoints_xy(self._x_bdry, self._y_bdry, self._N_pde)
        ...
        self._epochs, self._eqn, self._N_pde, self._setup_boundries, self._model, self._constraint, self._extra_ders, self._flag)
