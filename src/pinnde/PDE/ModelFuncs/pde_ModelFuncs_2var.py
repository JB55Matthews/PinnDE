...
def select_model_tx(t_bdry, x_bdry, t_order, inital_t, net_layers, net_units, constraint, setup_boundries):
  """
  ...
  if boundary_type == "periodic_timeDependent":
    if constraint == "soft":
      model = build_model_periodic_tx(t_bdry, net_layers, net_units)
    elif constraint == "hard":
      if t_order == 1:
        model = build_model_periodic_hardconstraint1_tx(t_bdry, inital_t[0], net_layers, net_units)
      elif t_order == 2:
    ...
    if constraint == "soft":
      ...
      if t_order == 1:
        ...
        model = build_model_dirichlet_hardconstraint2_tx(t_bdry, x_bdry, inital_t[0], inital_t[1], net_layers, net_units, setup_boundries[3], setup_boundries[4])
...
def select_model_xy(x_bdry, y_bdry, net_layers, net_units, constraint, setup_boundaries):
  ...
  Args:
    ...
    net_units (int): Number of units in each internal layer.
    constraint (string): Determines hard or soft constraints.
    setup_boundaries (boundary): boundary conditions set up from return of pde_Boundries_2var call.
  ...
  if boundary_type == "dirichlet_timeIndependent":
    ...
    model = build_model_periodic_xy(net_layers, net_units)
...
# Define the network
def build_model_periodic_tx(t, n_layers, n_units):
  """
  Used in solving:
  <ul>
  <li>solvePDE_tx with periodic boundries and soft constraints</li>
  </ul>
...
def build_model_standard(x_bdry, y_bdry, n_layers, n_units):
  ...
  Used in solving:
  <ul>
  <li>solvePDE_tx with dirichlet boundries and soft constraints</li>
  <li>solvePDE_tx with neumann boundries and soft constraints</li>
  <li>solvePDE_xy with dirichlet boundries and soft constraints</li>
  <li>solvePDE_xy with neumann boundries and soft constraints</li>
  </ul>
...
def build_model_periodic_xy(n_layers, n_units):
  """
  Used in solving:
  <ul>
  <li>solvePDE_xy with periodic boundries and soft constraints</li>
  <li>solvePDE_xy with periodic boundries and hard constraints</li>
  </ul>
...
# Define the network
def build_model_periodic_hardconstraint1_tx(t, u0, n_layers, n_units):
  """
  Used in solving:
  <ul>
  <li>solvePDE_tx with periodic boundries, hard constraints of a 
...
def build_model_periodic_hardconstraint2_tx(t, u0, ut0, n_layers, n_units):
  """
  Used in solving:
  <ul>
  <li>solvePDE_tx with periodic boundries, hard constraints of a 
...
def build_model_periodic_hardconstraint3_tx(t, u0, ut0, utt0, n_layers, n_units):
  """
  Used in solving:
  <ul>
  <li>solvePDE_tx with periodic boundries, hard constraints of a 
...
def build_model_hardconstraint_xy(x_bdry, y_bdry, n_layers, n_units, xleft_bound, xright_bound, ylower_bound, yupper_bound):
  ...
  Used in solving:
  <ul>
  <li>solvePDE_xy with dirichlet boundries and hard constraints</li>
  </ul>
  ...
  #1(xleftbound(0, 0)) + 0 + 1[ylowerbound(0, 0) - (1(yloerbound(0,0)) + 0)] + 0 = xleftboundry(0,0)
  # when x = xright (1), y=ylower(0)
  # 0 + xrightbound(1,0) + 1[ylowerbound(1,0) - (0 + ylowerbound(1, 0))] +0 = xrightboundry(1,0)
