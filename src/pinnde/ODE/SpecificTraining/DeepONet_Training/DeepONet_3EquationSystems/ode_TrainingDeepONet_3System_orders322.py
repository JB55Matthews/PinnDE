...
class HardConstraint2(nn.Module):
  t0: float
  tfinal: float
  u0: float
  ut0: float
  ...
  def __call__(self, inputs):
    t, nn = inputs
    return self.u0 + (t-self.t0)*self.ut0 + ((t-self.t0)/(self.tfinal-self.t0))**2*nn
...
class DeepONet(nn.Module):
  ...
  def __call__(self, t, u):
    ...
    trunk_x2 = trunk_net[:,3*self.units:5*self.units]
    trunk_x3 = trunk_net[:,5*self.units:]

    branch_x1 = branch_net[:,:3*self.units]
    branch_x2 = branch_net[:,3*self.units:5*self.units]
    branch_x3 = branch_net[:,5*self.units:]
    ...
    x2 = HardConstraint2(self.t0, self.tfinal, x2_0, x2t_0)([t, x2])
    x3 = HardConstraint2(self.t0, self.tfinal, x3_0, x3t_0)([t, x3])
...
def train_step(params, pdes, z, orders_case):
  ...
  def loss(params, t, z, orders_case):
    if (orders_case == 1): #u, x, y -> u, x, y
      ...
      ytt = w_tt(t, z, int(eqn3[-1]), params)
      uttt = w_ttt(t, z, int(eqn1[-1]), params)
    elif (orders_case == 2): #u, x, y -> x, u, y
      ...
      yt = w_t(t, z, int(eqn3[-1]), params)
      utt = w_tt(t, z, int(eqn2[-1]), params)
      ytt = w_tt(t, z, int(eqn3[-1]), params)
      xtt = w_tt(t, z, int(eqn1[-1]), params)
      xttt = w_ttt(t, z, int(eqn1[-1]), params)
    elif (orders_case == 3): #u, x, y -> y, u, x
      u = w0(t, z, int(eqn2[-1]), params)
      x = w0(t, z, int(eqn3[-1]), params)
      y = w0(t, z, int(eqn1[-1]), params)
      ut = w_t(t, z, int(eqn2[-1]), params)
      xt = w_t(t, z, int(eqn3[-1]), params)
      yt = w_t(t, z, int(eqn1[-1]), params)
      utt = w_tt(t, z, int(eqn2[-1]), params)
      ytt = w_tt(t, z, int(eqn1[-1]), params)
      xtt = w_tt(t, z, int(eqn3[-1]), params)
      yttt = w_ttt(t, z, int(eqn1[-1]), params)
...
def startTraining(eval_points, inits, orig_orders, t, N_sensors, sensor_range, epochs, net_layers, net_units, eqns):
  ...
  if ((orig_orders[0] == 3) and (orig_orders[1] == 2) and (orig_orders[2] == 2)):
    orders_case = 1 #u, x, y -> u, x, y
  elif ((orig_orders[0] == 2) and (orig_orders[1] == 3) and (orig_orders[2] == 2)):
    orders_case = 2 #u, x, y -> x, u, y
  elif ((orig_orders[0] == 2) and (orig_orders[1] == 2) and (orig_orders[2] == 3)):
    orders_case = 3 #u, x, y -> y, u, x
