...
class DeepONet(nn.Module):
  ...
  def __call__(self, t, u):
    ...
    x3 = HardConstraint3(self.t0, self.tfinal, x3_0, x3t_0, x3tt_0)([t, x3])
...
def defineCollocationPoints(t_bdry, N, sensor_range):
  ...
  zsensors = np.random.uniform(float(sensor_range[0]), float(sensor_range[1]), size=(N, 9))
...
def train_step(params, pdes, z):
  ...
  def loss(params, t, z):
    ...
    yttt = w_ttt(t, z, int(eqn3[-1]), params)
...
def startTraining(eval_points, inits, orig_orders, t, N_sensors, sensor_range, epochs, net_layers, net_units, eqns):
  ...
  params = deeponet.init(jax.random.PRNGKey(0), jnp.ones((10,)), jnp.ones((10,9)))
