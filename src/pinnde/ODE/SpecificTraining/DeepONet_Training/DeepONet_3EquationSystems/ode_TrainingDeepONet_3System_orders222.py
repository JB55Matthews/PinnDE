...
class DeepONet(nn.Module):
  ...
  def __call__(self, t, u):
    ...
    trunk_x1 = trunk_net[:,:2*self.units]
    trunk_x2 = trunk_net[:,2*self.units:4*self.units]
    trunk_x3 = trunk_net[:,4*self.units:]

    branch_x1 = branch_net[:,:2*self.units]
    branch_x2 = branch_net[:,2*self.units:4*self.units]
    branch_x3 = branch_net[:,4*self.units:]

    x1 = CombineBranches()(trunk_x1, branch_x1)
    x2 = CombineBranches()(trunk_x2, branch_x2)
    x3 = CombineBranches()(trunk_x3, branch_x3)

    x1 = HardConstraint2(self.t0, self.tfinal, x1_0, x1t_0)([t, x1])
