...
def train_step(params, pdes, l, r, lt, rt):
  ...
  def loss(params, t, l, r, lt, rt):
    ...
    left0_pred = u0(x0, l, r, lt, rt, params)
    eqn2 = left_0 - left0_pred

    x1 = np.repeat(np.expand_dims(t_1, axis=0), len(t), axis=0)
    right0_pred = u0(x1, l, r, lt, rt, params)
    eqn3 = right_0 - right0_pred

    xt0 = np.repeat(np.expand_dims(t_0, axis=0), len(t), axis=0)
    leftt0_pred = u_t(xt0, l, r, lt, rt, params)
    eqn4 = left_t0 - leftt0_pred
  
    xt1 = np.repeat(np.expand_dims(t_1, axis=0), len(t), axis=0)
    rightt0_pred = u_t(xt1, l, r, lt, rt, params)
    eqn5 = right_t0 - rightt0_pred
  
    return jnp.mean(eqn**2 + eqn2**2 + eqn3**2 + eqn4**2 + eqn5**2)
...
def startTraining(eval_points, inits, order, t, N_sensors, sensor_range, epochs, net_layers, net_units, eqn):
  ...
  global left_0, right_0, left_t0, right_t0, t_0, t_1
  left_0 = float(inits[0])
  right_0 = float(inits[1])
  left_t0 = float(inits[2])
  right_t0 = float(inits[3])
  t_0 = float(t[0])
  t_1 = float(t[1])
