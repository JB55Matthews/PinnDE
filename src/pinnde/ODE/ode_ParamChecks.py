...
def solveODE_IVP_ParamCheck(eqn, order, init_data, t_bdry, N_pde, epochs, net_units, net_layers, constraint):
    ...
    if (type(t_bdry) != list):
        ...
        raise ValueError("Left boundry must be less than right boundry")
...
def solveODE_BVP_ParamCheck(eqn, init_data, t_bdry, N_pde, epochs, order, net_units, net_layers, constraint):
    ...
    if (type(init_data) != list):
        ...
        raise ValueError("Initial boundry conditions must be [left boundry value, right boundry value]")
    
    elif ((order == 3) and (len(init_data) != 4)):
        raise ValueError("Initial boundry conditions must be [left boundry value, right boundry value, left boundry derivative value, right boundry derivative value]")
